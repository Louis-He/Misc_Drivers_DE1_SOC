...
{
    // printk(KERN_INFO "Key IRQ Handler called\n");
    // Update the terminal view
    printk(KERN_ALERT "\e[2J\e[10;10H------------\e[11;10H| %d%d:%d%d:%d%d |\e[12;10H------------\n", minute / 10, minute % 10, second / 10, second % 10, millisecond / 10, millisecond % 10);
...
{
    // printk(KERN_INFO "Before Increment: %02d:%02d:%02d\n", minute, second, millisecond);
...
static int __init initialize_handler(void)
{
    ...
    *HEX5_HEX4_ptr = seg7[0] << 8 | seg7[0];   // display 00
    ...
    if (ret) {
        printk(KERN_ALERT "Failed to request timer IRQ: %d\n", ret);
