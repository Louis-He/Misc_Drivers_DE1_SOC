...
unsigned char  SEGMENT_MESSAGE[] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x10, 0x54, 0x78, 0x79, 0x38, 0x0,
                                        0x6D, 0x5C, 0x39, 0x0,
	...
	*KEY_ptr = 0xF;
	...
	while (1) {
		...
		*(HEX5_4_ptr) = SEGMENT_MESSAGE[lowest_message_idx+1] | SEGMENT_MESSAGE[lowest_message_idx] << 8;
		...
		printf("\e[11;10H");
		fflush(stdout);
		printf("| %c%c%c%c%c%c |", MESSAGE[lowest_message_idx], MESSAGE[lowest_message_idx+1], 
				MESSAGE[lowest_message_idx+2], MESSAGE[lowest_message_idx+3], MESSAGE[lowest_message_idx+4], MESSAGE[lowest_message_idx+5]);
		fflush(stdout);
		printf("\e[12;10H");
		fflush(stdout);
		printf("----------");
		...
		if (lowest_message_idx > 19) {
			lowest_message_idx = 0;
		}
	}
